...
def build_samps(dutir,emo,term_set,term_dict, class_dict, doc_terms_list, doc_class_list,
    ...
    for k in range(len(doc_class_list)):
        ...
        samp_dict = {}
        #情感词个数
        samp_dict[len(term_set)+2] =0
        #表情符个数
        samp_dict[len(term_set)+3] =0
        #第k个样本中的每个词
        for term in doc_terms:
            if term_dict.has_key(term):
                #term_dict[term]=1  表示 term是 第一个特征
            ...
            #情感词
            if term in dutir :
                samp_dict[len(term_set)+2] += 1
            #表情符
            if term in emo:
                samp_dict[len(term_set)+3] += 1
...
def demo_hhh():
    print '读训练集、测试集...'
    #训练集
    doc_str_list_train, doc_class_list_train=read_text_f1('corpus/weibo_train_emotion')
    #测试集
    doc_str_list_test, doc_class_list_test = read_text_f3('corpus/label_weibo_test_emotion')
    doc_terms_list_train=get_doc_terms_list(doc_str_list_train)
    doc_terms_list_test =get_doc_terms_list(doc_str_list_test)
    class_set = get_class_set(doc_class_list_train)
    #查看class_set
    ...
    term_set_fs, term_score_dict = supervised_feature_selection(df_class, \
        df_term_class, fs_method, fs_num)
    term_set = term_set_fs
    print("词特征个数 : %r"%(len(term_set)))
    #加上20维度特征   11个标点符号
    feature_11=['！','，','、','：','~','；','…','—','%','“','”']
    for word in feature_11:
        if word not in term_set:
            term_set.append(word)
    print("加上标点符号后，特征个数：%r"%(len(term_set)))
    #读情绪词词典
    dutir=read_dic('dic/dic_DUTIR.txt')
    #读表情词典
    emo=read_dic('dic/EmoDic.txt')

    print '文本表示为向量 TF...'
    #把特征跟数字对应
    #增加情绪词特征和表情符特征
    term_dict = dict(zip(term_set, range(1, len(term_set)+3)))
    #类别与数字对应
    class_dict = dict(zip(class_set, range(1, 1+len(class_set))))
    term_weight = 'TF'
    samp_list_train, class_list_train = build_samps(dutir,emo,term_set,term_dict, class_dict, \
        doc_terms_list_train, doc_class_list_train, term_weight)
    samp_list_test, class_list_test = build_samps(dutir,emo,term_set,term_dict, class_dict, \
        doc_terms_list_test, doc_class_list_test, term_weight)
    ...
    # 返回train_x, train_y, test_x, test_y 
    # train_x、test_x返回的是词典，train_y、test_y是一维向量
    # return samp_list_train,class_list_train,samp_list_test, class_list_test
    # #返回特征的数量
    # return len(term_set)+2
